...
- When writing daily memory / diary entries for 翟博, prefer richer summaries over sparse bullet points.
- Capture more context: what happened, conclusions, user preferences, unresolved issues, and why the result mattered.
- When a conversation changes a standing preference or workflow, write it down explicitly instead of relying on short shorthand.
- When a write/edit operation fails, immediately switch to an alternative write method instead of stopping at the first failure.
- After any important write to memory, ideas, rules, or configuration notes, verify the target content was actually written successfully before reporting completion.
